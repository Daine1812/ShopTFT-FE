// === SỬA 1: TÁCH RIÊNG TYPE VÀ VALUE KHI IMPORT ===
...
// === SỬA 2: THAY THẾ 'enum' BẰNG 'const' VÀ 'type' ===
...
// Tự động tạo kiểu 'tft' | 'lol' | 'lienquan' từ const ở trên
...
interface Product {
  _id: string;
  category: ProductCategory; // Dùng kiểu mới
  name: string;
  description: string;
  image: string;
  price: number;
...
// === SỬA 3: THÊM KIỂU PROPS CHO TỪNG COMPONENT ===

// === Kiểu Props cho LoginForm ===
interface LoginProps {
  onLoginSuccess: (data: { access_token: string; user: User }) => void;
  onSwitchToRegister: () => void;
  setMessage: (message: string) => void;
}

function LoginForm({ onLoginSuccess, onSwitchToRegister, setMessage }: LoginProps) {
...
function RegisterForm({ onRegisterSuccess, onSwitchToLogin, setMessage }: RegisterProps) {
...
// === Kiểu Props cho VerifyForm ===
interface VerifyProps {
  email: string;
  onVerifySuccess: (data: { access_token: string; user: User }) => void;
  setMessage: (message: string) => void;
}

function VerifyForm({ email, onVerifySuccess, setMessage }: VerifyProps) {
...
function InitialPopup({ onShowLogin, onShowRegister, onGuest }: InitialPopupProps) {
  return (
    <Fragment>
...
function UserMenu({ user, isLoggedIn, onShowAuth, onLogout, onNavigate }: UserMenuProps) {
...
  useEffect(() => {
    // === SỬA: THÊM KIỂU 'MouseEvent' CHO 'event' ===
...
        <img
          src={isLoggedIn && user ? user.avatar || defaultAvatar : guestAvatar}
          alt="Avatar"
...
function UserProfile({ user, handleLogout, setUser }: UserProfileProps) {
...
      );
      ...
      // === SỬA: THÊM KIỂU 'User | null' CHO 'prevUser' ===
      setUser((prevUser: User | null) => (prevUser ? { ...prevUser, ...response.data } : null));
...
// === Kiểu Props cho DepositPage ===
interface DepositPageProps {
  user: User | null;
  // setUser: React.Dispatch<React.SetStateAction<User | null>>; // Lỗi TS6133: Bị khai báo nhưng không dùng, nên ta xóa đi
}

// === SỬA: XÓA 'setUser' KHỎI DANH SÁCH PROPS ===
function DepositPage({ user }: DepositPageProps) {
...
            <div className="form-group">
  _           <label>Nhập số tiền muốn nạp (VND):</label>
              <input
                type="number"
                value={amount}
...
function ProductList({ category, onBuyNow, onNavigate }: ProductListProps) {
...
  }
  
  // === SỬA: ĐỊNH NGHĨA KIỂU CHO categoryTitles ===
...
        <h2 className="section-title" style={{marginBottom: 0}}>
          {/* Lỗi TS7053 (dòng 890 cũ) đã được sửa ở đây */}
...
      setLoading(false);
      clearForm(); // Xóa form
  S     fetchAllProducts(); // Tải lại danh sách
      
    } catch (err) {
      console.error(err);
      setLoading(false);
...
        </div>
        <div className="form-group">
    _     <label>Tên đăng nhập (Tài khoản game) (*)</label>
...
                    <td style={{border: '1px solid #ddd', padding: '8px'}}>
            _         <img src={product.image} alt="icon" style={{width: '60px', height: '40px', objectFit: 'cover'}} />
                    </td>
...
// === Kiểu Props cho CategoryHomePage ===
interface CategoryHomePageProps {
  onCategorySelect: (category: ProductCategory) => void;
}

// === CẬP NHẬT: Component Trang chủ (Chỉ hiện Danh mục) ===
function CategoryHomePage({ onCategorySelect }: CategoryHomePageProps) {
...
i       `Giá: ${product.price.toLocaleString()} VND\n\n` +
...
          <>
            <RegisterForm
t             onRegisterSuccess={(email) => { // Lỗi 1493 (TS7006) đã được sửa ở đây
...
                  onNavigate={(page) => setCurrentPage(page)} // Lỗi 1567 (TS7006) đã được sửa ở đây
                  onShowAuth={(view) => { // Lỗi 1568 (TS7006) đã được sửa ở đây
                    setModalView(view);
                    setMessage('');
                    setShowModal(true);
                  }}
